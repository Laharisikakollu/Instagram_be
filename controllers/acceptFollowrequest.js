...
const acceptfollowrequest = async (req, res, next) => {
    ...
    if(req.body.accept == true)
    {
        ...
        const follower = await models.Followers.create({userId:payload.id,followerId:user.id})
           
        await models.Followrequest.destroy({where:{followingId:payload.id,userId:user.id}})
    }
    else
    {
        await models.Followrequest.destroy({where:{followingId:payload.id,userId:user.id}})
